...
d_fits_18 <- nest(df18, data = c(Temp, Average_speed)) %>%
...
ggplot(d_preds, aes(Temp, Average_speed)) +
  geom_point(aes(Temp, Average_speed), df18) +
...
ggplot(d_preds, aes(Temp, .fitted)) +
  ...
  geom_point(aes(Temp, Average_speed), df) +
  theme_bw(base_size = 12) +
  theme(legend.position = 'none') +
  labs(x = 'Temperature (?C)',
       y = 'Average speed (mm/s)',
