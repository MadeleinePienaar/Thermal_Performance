getwd()
setwd("Documents/R/Locomotion performance/Average speed/A18/Data/")
...
ggplot(df18, aes(Temp, Average_speed)) +
  geom_point() +
...
dev.off()
# fit Guassian model
d_fit_18 <- nest(df18, data = c(Temp, Average_speed)) %>%
...
ggplot() +
  ...
  geom_point(aes(Temp, Average_speed), df18, size = 2, alpha = 0.5) +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (?C)',
...
fit_nlsLM <- minpack.lm::nlsLM(Average_speed~gaussian_1987(temp = Temp, rmax, topt, a),
                               ...
                               lower = get_lower_lims(df18$Temp, df18$Average_speed, model_name = 'gaussian_1987'),
                               upper = get_upper_lims(df18$Temp, df18$Average_speed, model_name = 'gaussian_1987'),
...
write.csv(ci_extra_params, "Params_CI_Average_speed_A18.csv", sep = ",")
