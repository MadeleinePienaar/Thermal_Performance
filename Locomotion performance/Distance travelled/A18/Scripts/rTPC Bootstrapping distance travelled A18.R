...
ggplot(df18, aes(Temp, Distance_travelled)) +
  geom_point() +
...
dev.off()
# fit Guassian model
d_fit_18 <- nest(df18, data = c(Temp, Distance_travelled)) %>%
...
ggplot() +
  ...
  geom_point(aes(Temp, Distance_travelled), df18, size = 2, alpha = 0.5) +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (?C)',
...
fit_nlsLM <- minpack.lm::nlsLM(Distance_travelled~gaussian_1987(temp = Temp, rmax, topt, a),
                               ...
                               lower = get_lower_lims(df18$Temp, df18$Distance_travelled, model_name = 'gaussian_1987'),
                               upper = get_upper_lims(df18$Temp, df18$Distance_travelled, model_name = 'gaussian_1987'),
...
write.csv(ci_extra_params, "Params_CI_Distance_travelled_A18.csv", sep = ",")
