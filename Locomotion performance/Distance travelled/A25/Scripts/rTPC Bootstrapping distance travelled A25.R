...
dev.off()
# fit Guassian model
d_fit_25 <- nest(df25, data = c(Temp, Distance_travelled)) %>%
...
ggplot() +
  ...
  geom_point(aes(Temp, Distance_travelled), df25, size = 2, alpha = 0.5) +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (?C)',
...
fit_nlsLM <- minpack.lm::nlsLM(Distance_travelled~gaussian_1987(temp = Temp, rmax, topt, a),
                               ...
                               lower = get_lower_lims(df25$Temp, df25$Distance_travelled, model_name = 'gaussian_1987'),
                               upper = get_upper_lims(df25$Temp, df25$Distance_travelled, model_name = 'gaussian_1987'),
...
write.csv(ci_extra_params, "Params_CI_Distance_travelled_A25.csv", sep = ",")
