...
dev.off()
# fit Guassian model
d_fit_32 <- nest(df32, data = c(Temp, Distance_travelled)) %>%
  mutate(gaussian = map(data, ~nls_multstart(Distance_travelled~gaussian_1987(temp = Temp, rmax, topt, a),
                                             data = .x,
                                             iter = c(4,4,4),
...
ggplot() +
  ...
  geom_point(aes(Temp, Distance_travelled), df32, size = 2, alpha = 0.5) +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (?C)',
...
fit_nlsLM <- minpack.lm::nlsLM(Distance_travelled~gaussian_1987(temp = Temp, rmax, topt, a),
                               data = df32,
                               start = coef(d_fit_32$gaussian[[1]]),
                               lower = get_lower_lims(df32$Temp, df32$Distance_travelled, model_name = 'gaussian_1987'),
                               upper = get_upper_lims(df32$Temp, df32$Distance_travelled, model_name = 'gaussian_1987'),
...
p1 <- ggplot() +
  ...
  labs(x = 'Temperature (?C)',
       ...
       title = 'Bootstrapped confidence intervals: Distance travelled \n across temperatures of 32?C acclimated PSHB')
p1
...
p2 <- ggplot() +
  ...
  labs(x = 'Temperature (?C)',
       y = 'Distance travelled (mm)',
       title = 'Bootstrapped predictions: Distance travelled \n across temperatures of 32?C acclimated PSHB')
...
write.csv(ci_extra_params, "Params_CI_Distance_travelled_A32.csv", sep = ",")
