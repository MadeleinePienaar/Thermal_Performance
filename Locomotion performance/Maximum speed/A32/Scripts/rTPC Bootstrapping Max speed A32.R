...
dev.off()
# fit model
d_fit_32 <- nest(df32, data = c(Temp, Max_speed)) %>%
...
ggplot() +
  ...
  geom_point(aes(Temp, Max_speed), df32, size = 2, alpha = 0.5) +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (?C)',
...
fit_nlsLM <- minpack.lm::nlsLM(Max_speed~ratkowsky_1983(temp = Temp, tmin, tmax, a, b),
                               data = df32,
                               start = coef(d_fit_32$ratkowsky[[1]]),
                               lower = get_lower_lims(df32$Temp, df32$Max_speed, model_name = 'ratkowsky_1983'),
                               upper = get_upper_lims(df32$Temp, df32$Max_speed, model_name = 'ratkowsky_1983'),
...
write.csv(ci_extra_params, "Params_CI_Max_speed_A32.csv", sep = ",")
