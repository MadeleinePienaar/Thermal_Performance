...
ggplot(df18, aes(Temp, Time_travelling)) +
  ...
  labs(x = 'Temperature (?C)',
       y = 'Time spent travelling (mins over 30 mins)',
       title = 'A18: Time spent travelling across temperatures\n log(time+1)')

dev.off()
# fit Beta model
d_fit_18 <- nest(df18, data = c(Temp, Time_travelling)) %>%
...
ggplot() +
  ...
  geom_point(aes(Temp, Time_travelling), df18, size = 2, alpha = 0.5) +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (?C)',
...
fit_nlsLM <- minpack.lm::nlsLM(Time_travelling~thomas_2017(temp = Temp, a,b,c,d,e),
                               data = df18,
                               start = coef(d_fit_18$thomas2[[1]]),
                               lower = get_lower_lims(df18$Temp, df18$Time_travelling, model_name = 'thomas_2017'),
                               upper = get_upper_lims(df18$Temp, df18$Time_travelling, model_name = 'thomas_2017'),
...
ci_bact <- bind_rows(ci_bact1, ci_bact2) %>%
...
write.csv(ci_extra_params, "Params_CI_Time_travelling_A18_thomas2.csv", sep = ",")
