...
#Detect outliers
boxplot(df25$Time_travelling)$out
outliers <- boxplot(df25$Time_travelling, plot=FALSE)$out
print(outliers)
...
ggplot(df25, aes(Temp, Time_travelling)) +
  ...
  labs(x = 'Temperature (?C)',
       y = 'Time spent travelling (minutes over 30 minutes)',
       title = 'Time spent travelling across temperatures')

dev.off()
# fit thomas1 model
d_fit_25 <- nest(df25, data = c(Temp, Time_travelling)) %>%
  mutate(thomas2 = map(data, ~nls_multstart(Time_travelling~thomas_2017(temp = Temp, a,b,c,d,e),
                                            data = .x,
                                            iter = c(3,3,3,3,3),
         ...
         preds =  map2(thomas2, new_data, ~augment(.x, newdata = .y)))
...
ggplot() +
  ...
  geom_point(aes(Temp, Time_travelling), df25, size = 2, alpha = 0.5) +
  theme_bw(base_size = 12) +
  labs(x = 'Temperature (?C)',
       y = 'Time spent travelling (log(time+1))',
       title = 'A25: Time spent travelling across temperatures')


# refit model using nlsLM
fit_nlsLM <- minpack.lm::nlsLM(Time_travelling~thomas_2017(temp = Temp, a,b,c,d,e),
                               data = df25,
                               start = coef(d_fit_25$thomas2[[1]]),
                               lower = get_lower_lims(df25$Temp, df25$Time_travelling, model_name = 'thomas_2017'),
                               upper = get_upper_lims(df25$Temp, df25$Time_travelling, model_name = 'thomas_2017'),
...
write.csv(ci_extra_params, "Params_CI_Time_travelling_A25_thomas2.csv", sep = ",")
